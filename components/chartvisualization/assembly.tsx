...
export default function Assembly() {
    ...
    const fetchStampingPress =  () => {
      ...
      eventSource.onmessage = (event) => {
          try {
              ...
              setassembly((prevData) => [...prevData, newData]); // Append new data point
          } catch (err) {
  ...
  const timestamps = assembly.map((item) =>
  ...
  const assemblyData = {
    ...
    datasets: [
      {
        label: "assembly machine utilization",
        data: assembly.map((item) => item?.KPI_Value || 0),
