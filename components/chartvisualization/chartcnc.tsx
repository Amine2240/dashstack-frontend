...
export default function Chartcnc() {
  ...
  useEffect(() => {
    ...
    const fetchStampingPress =  () => {
      ...
      eventSource.onmessage = (event) => {
          try {
              ...
              setcncMachineUtilization((prevData) => [...prevData, newData]); // Append new data point
          } catch (err) {
    ...
    const timestamps = cncMachineUtilization.map((item) =>
      ...
      const cncData = {
        ...
        datasets: [
          {
            label: "cnc machine utilization",
            data: cncMachineUtilization.map((item) => item?.KPI_Value|| 0),
