...
export default function Chartpainting() {
    ...
    const fetchStampingPress =  () => {
      ...
      eventSource.onmessage = (event) => {
          try {
              ...
              setpaintingRobot((prevData) => [...prevData, newData]); // Append new data point
          } catch (err) {
  ...
  const timestamps = paintingRobot.map((item) =>
  ...
  const paintingData = {
    ...
    datasets: [
      {
        label: "painting robot performence",
        data: paintingRobot.map((item) => item?.KPI_Value || 0),
