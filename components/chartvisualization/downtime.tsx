...
export default function Downtime() {
  ...
  useEffect(() => {
    ...
    const fetchStampingPress =  () => {
      ...
      const eventSource = new EventSource('http://localhost:4000/api/kpi/machine-downtime');
      ...
      eventSource.onmessage = (event) => {
          try {
              ...
              setdowntime((prevData) => [...prevData, newData]); // Append new data point
          } catch (err) {
    ...
    const timestamps = downtime.map((item) =>
        ...
        datasets: [
          {
            ...
            data: downtime.map((item) => item?.KPI_Value|| 0),
